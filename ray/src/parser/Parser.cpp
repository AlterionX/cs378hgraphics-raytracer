...
AreaLightCirc* Parser::parseAreaLightCirc(Scene* scene) {
	glm::dvec3 direction;
	glm::dvec3 position;
	glm::dvec3 color;
	double radius;
	...
	for (;; ) {
		...
		case RADIUS:
			if (hasRadius)
				throw SyntaxErrorException("Repeated 'radius' attribute", _tokenizer);
		...
		case RBRACE:
			if (!hasRadius)
				throw SyntaxErrorException("Expected: 'radius'", _tokenizer);
			if (!hasColor)
				throw SyntaxErrorException("Expected: 'color'", _tokenizer);
			if (!hasPosition)
				throw SyntaxErrorException("Expected: 'position'", _tokenizer);
			...
			_tokenizer.Read(RBRACE);
			return new AreaLightCirc(scene, position, color, constantAttenuationCoefficient,
				linearAttenuationCoefficient, quadraticAttenuationCoefficient, direction, radius);
...
AreaLightRect* Parser::parseAreaLightRect(Scene* scene) {
	glm::dvec3 direction;
	glm::dvec3 position;
	glm::dvec3 color;
	double width;
	...
	for (;; ) {
		...
		case WIDTH:
			if (hasWidth)
				throw SyntaxErrorException("Repeated 'width' attribute", _tokenizer);
			width = parseScalarExpression();
			hasWidth = true;
			break;

		case HEIGHT:
			if (hasHeight)
				throw SyntaxErrorException("Repeated 'height' attribute", _tokenizer);
			height = parseScalarExpression();
			hasHeight = true;
			break;
		...
		case UPDIR:
			if (hasUpDir)
				throw SyntaxErrorException("Repeated 'updir' attribute", _tokenizer);
			updir = parseVec3dExpression();
			hasUpDir = true;
			break;
		...
		case RBRACE:
			if (!hasWidth)
				throw SyntaxErrorException("Expected: 'width'", _tokenizer);
			if (!hasHeight)
				throw SyntaxErrorException("Expected: 'height'", _tokenizer);
			if (!hasUpDir)
				throw SyntaxErrorException("Expected: 'updir'", _tokenizer);
			if (!hasColor)
				throw SyntaxErrorException("Expected: 'color'", _tokenizer);
			...
			if (!hasDirection)
				throw SyntaxErrorException("Expected: 'direction'", _tokenizer);
			_tokenizer.Read(RBRACE);
			return new AreaLightRect(scene, position, color, constantAttenuationCoefficient,
				linearAttenuationCoefficient, quadraticAttenuationCoefficient, direction,
				width, height, updir);
	...
	for (;; ) {
		...
		case RBRACE:
			...
			return new SpotLight(scene, position, color, constantAttenuationCoefficient,
				linearAttenuationCoefficient, quadraticAttenuationCoefficient, direction, radius, angle);
...
Material* Parser::parseMaterial(Scene* scene, const Material& parent) {
	...
	for (;; ) {
		...
		case GLOSS:
			mat->setGloss(parseScalarMaterialParameter(scene));
			break;
