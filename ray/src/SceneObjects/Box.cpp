...
bool Box::intersectLocal(ray& r, isect& i) const {
    ...
    if(bestIndex < 3) {
        i.setUVCoordinates( glm::dvec2( 0.5 - intersect_point[ min(i1, i2) ], 0.5 + intersect_point[ max(i1, i2) ] ) );
...
void Box::intersectLocalList(ray& r, std::vector<isect>& iv) const {
    ...
    for(int it = 0; it < 6; it++) { 
        ...
        double t = ((it/3) - 0.5 - p[mod0]) / d[mod0];
        if(t < RAY_EPSILON) continue;

        int mod1 = (it + 1) % 3;
        int mod2 = (it + 2) % 3;
        double x = p[mod1] + t * d[mod1];
        double y = p[mod2] + t * d[mod2];
        ...
        if (x <= 0.5 && x >= -0.5 && y <= 0.5 && y >= -0.5) {
            ...
            glm::dvec3 intersect_point = r.at(i);
            i.setUVCoordinates( glm::dvec2( 0.5 + ((it < 3) ? -1.0 : 1.0) * intersect_point[ min(mod1, mod2) ], 0.5 + intersect_point[ max(mod1, mod2) ] ) );
            i.setN(computeNormal(it, i));
