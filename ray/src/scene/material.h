...
public:
	...
	Material(const glm::dvec3& e, const glm::dvec3& a, const glm::dvec3& s,
		const glm::dvec3& d, const glm::dvec3& r, const glm::dvec3& t, double sh, double in, double gl)
		: _ke(e), _ka(a), _ks(s), _kd(d), _kr(r), _kt(t), _bump(glm::dvec3(0.0, 0.0, 0.0)),
		_shininess(glm::dvec3(sh, sh, sh)), _index(glm::dvec3(in, in, in)), _gloss(gl) {
		setBools();
	}
	...
	double gloss(const isect& i) const { return _gloss.intensityValue(i); }
	...
	void setGloss(double gloss) { _gloss.setValue(gloss); }
